...
3. **Run Simulation:** Compute curvature ($\kappa$), radius ($R$), and bending angle ($\theta$).
...
2. **Run Simulation:** Compute parallel and transverse swelling strains, and swelling anisotropy ratio ($A_s$).
3. **Observe:** Vary fiber orientation angle ($\theta_f$) and plot net bending angle vs $\theta_f$. Identify the orientation giving maximum out-of-plane bending.

## Sub-Calc C: Print Path as Shape Programme
**Objective:** Program transformation geometry using print direction.

1. **Set Angles:** Input top layer print angle ($\theta_{top}$) and bottom layer print angle ($\theta_{bottom}$).
2. **Run Simulation:** Compute effective CTE ($\alpha_{eff}$) for each layer and programmed curvature.
3. **Observe:** Test angle combinations (e.g., $0^\circ/90^\circ$, $45^\circ/-45^\circ$) to identify which produces max flat bending, twisting, or helical shape.
...
2. **Run Simulation:** Compute characteristic diffusion time ($t_{diff}$) and time to 90% swelling equilibrium ($t_{90}$).
3. **Observe:** Plot $t_{90}$ vs $L^2$. Confirm the scaling law where thinner layers respond exponentially faster.

## Sub-Calc E: Blocking Force & Work Output
**Objective:** Compute blocking force and energy density of the actuator.

1. **Run Simulation:** Compute free deflection from Sub-Calc A and resulting blocking force ($F_{block}$).
2. **Observe:** Record work output ($W$) and energy density ($U_{act}$) for different humidity levels. Compare work output to other actuators.
