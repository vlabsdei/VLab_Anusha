...
(function () {
  ...
  function refresh() {
    ...
    BP.put(document.getElementById('resRecoveryPenalty'), '-' + (95 - rr).toFixed(0) + ' % R_r to ' + rr.toFixed(0) + ' %');

    const li = document.getElementById('liveInsight');
    if (li) li.innerHTML = '<strong>Live Insight:</strong> The road cools with τ_cool = <strong>' + tau.toFixed(1) + ' s</strong> and lingers <strong>' + tx.toFixed(1) + ' s</strong> in the 60-170 °C growth window, reaching X_c = <strong>' + (100 * xc).toFixed(0) + '%</strong>. That stiffens it to <strong>' + E.toFixed(2) + ' GPa</strong> (+' + (100 * (E / E_AMORPH - 1)).toFixed(0) + '%) but pins the network, cutting shape-memory recovery to ' + rr.toFixed(0) + '%.';
...
(function () {
  ...
  function updateEq(e, sr, d) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div><strong>Delamination:</strong> σ_res &gt; σ_bond (30 MPa), so ' + (sr > SIGMA_BOND ? '<span style="color:#B42318;font-weight:700;">peels off bed</span>' : '<span style="color:#15803D;font-weight:700;">stays bonded</span>') + '</div>' +
...
(function () {
  ...
  function updateEq(sTh, AR, tm) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div><strong>Weakest plane:</strong> θ_min = ' + tm.th.toFixed(0) + '°, so σ = ' + tm.sig.toFixed(1) + ' MPa &nbsp;(shear-dominated, not 90°)</div>' +
...
(function () {
  ...
  function updateEq(mu, sig, dRSS, dWC, cpk) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div style="color:#8b929b;font-size:0.92em;">RSS assumes independent errors; correlated 4D errors mean worst-case is the honest bound.</div></div>';
  }
