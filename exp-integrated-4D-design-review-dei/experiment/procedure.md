...
This capstone bench reviews a 4D-printed active structure the way a design team would, one decision at a time. Each of the five sub-calculators is gated behind a **Start Experiment** panel; set your parameters first, then start the solver. Work through the sub-calcs in order.
...
2. **Set the target radius:** Drag **Target radius R** (3-20 mm). The predicted curvature is &kappa;<sub>pred</sub> = 1/R.
3. **Set the bilayer geometry:** Adjust the **Thickness ratio m = h<sub>1</sub>/h<sub>2</sub>** (0.1-4), the **Mismatch strain &Delta;&epsilon;** (0.5-15 %), and the **Modulus ratio n = E<sub>1</sub>/E<sub>2</sub>** (0.1-10).
...
3. **Set the part and printer:** Adjust **Part mass** (5-200 g) and **Printer power** (50-300 W).
...
## Sub-Calc D: Lab to Manufacturing (Scalability)
...
1. **Set the part volume:** Drag **Part volume V<sub>part</sub>** (1-100 cm&sup3;). This sets the print time via the volumetric FDM rate.
2. **Set the production volume:** Adjust **Production volume N** (logarithmic, 1 to 10<sup>4</sup>).
3. **Set the moulding economics:** Adjust **Mould tooling cost C<sub>mould</sub>** ($1,000-$50,000) and the **Machine rate** ($5-$120/hr).
...
2. **Set a target rung (optional):** Drag **Target TRL** (1-9) to mark where you want the technology to reach.
