...
(function () {
  ...
  function refresh() {
    ...
    MATS.forEach((m) => {
      ...
      F.ctx.fillStyle = isWin ? '#9A3412' : '#16223a'; F.ctx.font = (isWin ? '700 ' : '600 ') + '10px "IBM Plex Mono", monospace';
      F.ctx.textAlign = 'center'; F.ctx.textBaseline = 'bottom'; F.ctx.fillText(m.name, X, Y - rad - 3);
    });
...
(function () {
  ...
  function refresh() {
    ...
    const li = document.getElementById('liveInsight');
    if (li) {
      if (pass) li.innerHTML = '<strong>Live Insight:</strong> Predicted κ_pred = ' + s.kPred.toFixed(0) + ' m⁻¹ (design 1/R), simulated κ_sim = ' + s.kSim.toFixed(0) + ' m⁻¹ &mdash; a <strong>' + s.err.toFixed(1) + '%</strong> shape error, <strong>inside</strong> the 10% acceptance band. The ' + (model === 'smp' ? 'SMP-recovery' : 'Timoshenko bilayer (Exp 2)') + ' model is validated for these parameters.';
      else li.innerHTML = '<strong>⚠ Verification Failed:</strong> κ_sim = ' + s.kSim.toFixed(0) + ' m⁻¹ vs κ_pred = ' + s.kPred.toFixed(0) + ' m⁻¹ &mdash; ε_shape = <strong>' + s.err.toFixed(1) + '%</strong> exceeds 10%. Likely culprit: <strong>' + culprit(s) + '</strong>. Reduce Δε back into the small-strain range or bring m toward 1.';
    }
    updateEq(s, pass);
  }
...
(function () {
  ...
  const BANDS = [{ lo: 1, hi: 3, name: 'Basic / analytical', c: 'rgba(139,146,155,0.14)' }, { lo: 4, hi: 6, name: 'Lab → relevant env', c: 'rgba(30,64,175,0.10)' }, { lo: 7, hi: 9, name: 'System / flight', c: 'rgba(21,128,61,0.12)' }];
  ...
  function refresh() {
    ...
    // band labels
    BANDS.forEach((b) => { const ym = yOf((b.lo + b.hi) / 2); ctx.save(); ctx.fillStyle = '#8b929b'; ctx.font = '9px "IBM Plex Mono", monospace'; ctx.textAlign = 'left'; ctx.textBaseline = 'middle'; ctx.fillText(b.name, pl + 6, ym); ctx.restore(); });
    ...
    const li = document.getElementById('liveInsight');
    if (li) li.innerHTML = '<strong>Live Insight:</strong> <strong>' + A.name + '</strong> sits at <strong>TRL ' + A.trl + '</strong> (' + (A.trl <= 3 ? 'basic/analytical' : A.trl <= 6 ? 'lab → relevant environment' : 'system/flight') + '). The gap to TRL ' + next + ' is blocked by: <strong>' + A.barrier + '</strong> — which traces back to ' + A.source + '. ' + (target > A.trl ? 'Reaching TRL ' + target.toFixed(0) + ' needs ' + (target - A.trl) + ' more rung(s).' : '');
    updateEq(A, next);
  }
  function updateEq(A, next) {
    const box = document.getElementById('eqBox'); if (!box) return;
    box.innerHTML = '<div style="line-height:1.85;">' +
      '<div><strong>TRL scale:</strong> 1–3 basic principles / analytical proof · 4–6 lab → relevant-environment validation · 7–9 system prototype / operational / flight-proven</div>' +
