...
(function() {
    ...
    function updateEquations(atNi, Ms, Mf, As, Af, T, xm) {
        equationsContainer.innerHTML = `
            ...
            <div style="font-size: 1.15em; background: #ffffff; padding: 10px 14px; border-radius: 6px; border: 1px solid #e2e8f0; font-family: monospace; font-weight: bold; margin-bottom: 12px; color: #E2570F;">
            ...
            <div style="font-size: 1.15em; background: #ffffff; padding: 10px 14px; border-radius: 6px; border: 1px solid #e2e8f0; font-family: monospace; font-weight: bold; color: #1E40AF;">
    ...
    function drawPlots(activeTemp, As, Af, Ms, Mf) {
        ...
        LabLegend.attach(plotCanvas, [
            { color: '#E2570F', text: 'Heating: endothermic peak' },
            { color: '#1E40AF', text: 'Cooling: exothermic peak' },
            { color: '#10b981', text: 'Hysteresis window' },
            { color: '#cbd5e1', text: `A_s marker = ${As.toFixed(0)} deg C` },
            { color: '#cbd5e1', text: `A_f marker = ${Af.toFixed(0)} deg C` }
        ], 'tr');
    }
    ...
    function init3D() {
        ...
        const wireMat = new THREE.MeshStandardMaterial({ color: 0x1E40AF, metalness: 0.55, roughness: 0.3 });
    ...
    btnRun.addEventListener('click', () => {
        ...
        const interval = setInterval(() => {
            ...
            if (animTemp >= targetTemp) {
                ...
                btnRun.style.background = "linear-gradient(135deg, #B8410F, #E2570F)";
...
(function() {
    ...
    function updateUI() {
        ...
        resComparison.innerHTML = `Steel: <b>0.001</b> | Rubber: <b>0.100</b><br><span style="color:#E2570F">Nitinol is <b>${(damping / 0.001).toFixed(0)}x</b> higher than structural steel!</span>`;
    ...
    function updateEquations(relTemp, T, plateau, W, damping, sigMax, eMax) {
        if (relTemp < 0) {
            equationsContainer.innerHTML = `
                ...
                <div style="font-size: 1.15em; background: #ffffff; padding: 10px 14px; border-radius: 6px; border: 1px solid #e2e8f0; font-family: monospace; font-weight: bold; margin-bottom: 12px; color: #1E40AF;">
...
(function() {
    ...
    function drawPlots() {
        ...
        LabLegend.attach(plotCanvas, [
            { color: '#E2570F', text: 'Wire temperature T(t)' },
            { color: '#cbd5e1', dash: true, text: `A_s = ${As} deg C` },
            { color: '#cbd5e1', dash: true, text: `A_f = ${Af} deg C` }
        ], 'tl');
    }
...
(function() {
    ...
    function updateUI() {
        ...
        resComparison.innerHTML = `SMP: <b>5 J/kg</b> | Pneumatic: <b>100 J/kg</b> | Hydraulic: <b>1000 J/kg</b><br><span style="color:#E2570F">Nitinol specific work is <b>${(specWork / 100.0).toFixed(0)}x</b> higher than Pneumatics!</span>`;
    ...
    function init3D() {
        ...
        const springMat = new THREE.MeshStandardMaterial({ color: 0xb9c0cc, metalness: 0.9, roughness: 0.28 });
        const wireMat = new THREE.MeshStandardMaterial({ color: 0xE2570F, metalness: 0.5, roughness: 0.35 });
...
(function() {
    ...
    function updateUI() {
        ...
        if (isAnimating) {
            ...
            liveInsight.innerHTML = `<strong>Fatigue Insight:</strong> ${pct.toFixed(0)}% of fatigue life consumed - ${curCycle.toLocaleString()} of ~${cycles.toLocaleString(undefined, { maximumFractionDigits: 0 })} cycles.`;
        } else if (curCycle > 0) {
    ...
    function init3D() {
        ...
        const stripeMat = new THREE.MeshStandardMaterial({ color: 0xE2570F, metalness: 0.5, roughness: 0.4, emissive: 0x3a0713, emissiveIntensity: 0.25 });
