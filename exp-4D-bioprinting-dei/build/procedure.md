...
Welcome to the 4D Bioprinting Virtual Laboratory. This lab walks the printed construct from the syringe to a deployed device: first whether the ink extrudes, then whether it sets, whether the cells inside survive, how long the scaffold lasts, and finally a capstone that ties it all into a self-folding tracheal stent. Each sub-calculator is gated — set your parameters, click **Start Experiment**, then run. Work through A to E in order.

---

## Sub-Calc A: Bioink Viscosity & the Printability Window
**Objective:** Tune a shear-thinning ink so its viscosity at the nozzle wall lands inside the 1&ndash;100 Pa&middot;s printable band, and check the recovery time that lets a printed road hold its shape.

1. Click **Start Experiment** to arm the viewport, then set **K (Pa&middot;s&#8319;)** — the overall thickness / consistency index (2&ndash;80).
2. Set the **flow index n** (0.2&ndash;1.0). Anything below 1 is shear-thinning; the log&ndash;log slope you get is n&minus;1.
3. Adjust **v_print (mm/s)** (2&ndash;40) and the **thixotropic recovery time &tau; (s)** (0.5&ndash;20).
...
6. In Readouts, note the log&ndash;log slope, the 400 µm shear rate and viscosity, how many nozzles print, and the 90% shape-recovery time (t90). Use **Reset Ink** to start over.
...
1. **Start Experiment**, then set the **plate temperature T (&deg;C)** (4&ndash;45). Warmer gels faster through the Arrhenius scaling of k<sub>Av</sub>.
2. Set the **Avrami exponent m** (1&ndash;4) — the nucleation/growth dimensionality that shapes the sigmoid.
3. Set the **plateau modulus G&prime;&infin; (Pa)** (50&ndash;800), the final network stiffness.
...
1. **Start Experiment**, then set **v_print (mm/s)** (2&ndash;40).
2. Set the **nozzle viscosity &eta; (Pa&middot;s)** at print shear (0.05&ndash;2.0) and the **nozzle length L_nozzle (mm)** (0.5&ndash;10), which fixes the residence time t_res = L/v.
3. Set the **cell shear sensitivity k_kill (Pa&#8315;&sup1;s&#8315;&sup1;)** (0.002&ndash;0.03; ~0.01 for mammalian cells).
...
2. Set the **initial porous modulus E&#8320; (MPa)** (0.3&ndash;5) — the as-printed stiffness, deliberately MPa-scale for a porous strut network, not GPa bulk.
3. Set the **tissue stiffness threshold E_min (MPa)** (0.02&ndash;1.0), the load-bearing floor.
...
1. **Start Experiment**, then set the **trachea radius R_tube (mm)** (5&ndash;14). This fixes &kappa;_target = 1/R_tube, and the print aims 20% past it.
2. Set the **layer thickness ratio m = h&#8321;/h&#8322;** (0.2&ndash;3), the active/passive split, and the **programmed mismatch strain &Delta;&epsilon;** (5&ndash;40%).
...
4. Click **Design &amp; Deploy Stent**. The flat sheet folds to &kappa;_printed and holds — but if the SMP fails the T<sub>g</sub> gate, watch it spring back open, dramatizing premature room-temperature deployment.
