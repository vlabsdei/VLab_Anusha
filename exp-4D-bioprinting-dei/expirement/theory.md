...
* n is the flow index. For n &lt; 1 the ink is shear-thinning; on a log-log plot &eta; vs &gamma;&#775; is a straight line of slope (n&minus;1).
...
<p align="center"><em>Figure 1: The shear-thinning power law is a straight line of slope n&minus;1 on log-log axes; each nozzle sits at its own wall shear rate, and only those landing inside the 1-100 Pa&middot;s band print cleanly (Sub-Calc A).</em></p>

---

## 4. Thermal Gelation Kinetics (Sub-Calc B)
A freshly printed layer is still liquid, and it must gel fast enough to carry the next layer before it slumps. Isothermal gelation is modeled with **Avrami kinetics**, borrowed from crystallization theory:

<div align="center" style="font-size: 1.1em; font-weight: bold;">&alpha;(t) = 1 &minus; exp(&minus;k<sub>Av</sub> &middot; t<sup>m</sup>)</div>

Here &alpha; is the fraction converted from sol to gel, m is the Avrami exponent (nucleation/growth dimensionality, 1-4), and k<sub>Av</sub> is the rate constant. The **gel point** is taken at &alpha; = &frac12;, which inverts to a closed form:
...
k<sub>kill</sub> &asymp; 0.01 Pa&#8315;&sup1;s&#8315;&sup1; lumps the cell type and shear mode into one screening coefficient; the target is survival S &gt; 85%. This is the **resolution-viability trade-off** unique to bioprinting: the 200 µm tip prints the finest features but drives &tau;<sub>wall</sub> highest, while the 610 µm tip is gentle but coarse. The model is a &plusmn;20% screen, so a real build must still be confirmed with LIVE/DEAD staining.
...
<p align="center"><em>Figure 3: Survival decays exponentially with wall shear stress; the fine 200 µm nozzle sits deepest in the danger zone while the coarse 610 µm tip stays above the 85% target, which is the resolution-viability trade-off (Sub-Calc C).</em></p>
...
Stiffness, though, falls *faster* than mass. A porous strut network follows the Gibson-Ashby style scaling E &prop; M<sup>4/3</sup>, so:
...
The three model polymers span three timescales: PLGA (k<sub>deg</sub> = 0.02/day, weeks), PLA (0.002/day, months), and PCL (0.0005/day, years). Note E<sub>0</sub> here's the *porous* scaffold modulus (MPa-scale), not the bulk polymer (GPa). The design goal is to match t<sub>fail</sub> to a tissue's regeneration time (skin ~21 d, bone ~90 d, cartilage ~150 d).
...
Geometry alone isn't enough. The SMP must also clear three **deployment safety gates**: shape fixity R<sub>f</sub> &gt; 95%, shape recovery R<sub>r</sub> &gt; 90%, and a glass transition T<sub>g</sub> = 45-50 &deg;C. That T<sub>g</sub> window is the subtle one: a PCL-SMP at T<sub>g</sub> = 35 &deg;C is only 2 &deg;C below body temperature, so the stent could deploy prematurely during room-temperature handling. The PLA-SMP (T<sub>g</sub> 58 &deg;C, R<sub>f</sub> 97%, R<sub>r</sub> 92%) clears every gate.
...
## 8. Two Deadlines: Gelation Before Print, Degradation After

A 4D bioprinted construct is bracketed by two clocks, and the living window sits between them. During
printing the ink must not gel before the layer is placed: gelation time is t<sub>gel</sub> =
(ln2/k<sub>Av</sub>)<sup>1/m</sup>. After printing the scaffold degrades as M(t) = M<sub>0</sub>
e<sup>&minus;k<sub>deg</sub>t</sup>, and its stiffness follows down until t<sub>fail</sub> =
ln(E<sub>0</sub>/E<sub>min</sub>)/((4/3)k<sub>deg</sub>). The time between "set" and "too weak" is the
functional lifetime, the fourth dimension of a living part, and cell survival S = exp(&minus;k<sub>kill</sub>
&middot; &tau;<sub>wall</sub> &middot; t<sub>res</sub>) trades against it through print speed.

<div align="center" style="font-size: 1.1em; font-weight: bold;">stent passes only if it deploys (R<sub>f</sub>, R<sub>r</sub>, T<sub>g</sub> gates) BEFORE its mechanical lifetime ends</div>

**One-shot, not cyclic.** The self-deploying stent is a single transformation, but it must still hold
&kappa;<sub>printed</sub> = 1.2&kappa;<sub>target</sub> against the airway after degradation has begun, so
the deployment gates and the lifetime clock are checked together, and either can fail the part.

**Bench bridge.** The nozzle wall shear &tau;<sub>wall</sub> = 4&eta;Q/(&pi;R&sup3;) is the same
Poiseuille physics measured in the *Redwood viscometer* experiment (Sir C.R. Reddy College of Engineering).
Mapped page in `screenshots_references/`.
