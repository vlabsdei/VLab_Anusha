...
(function () {
  ...
  function updateEq(s) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div style="color:#8b929b;font-size:0.92em;">The constant-moment-arm linear model holds only for θ ≲ 30°; the rolloff term reproduces the calibrated 20 kPa→28°, 80 kPa→94° saturation.</div></div>';
  }
...
(function () {
  ...
  function refresh() {
    ...
    BP.put(document.getElementById('resEnergySaved'), locked ? pPneu.toFixed(2) + ' W → 0 W' : '0 W (not locked)', locked ? '#15803D' : '#B42318');
  ...
  function updateEq(SR, thB, thHeld, locked, pPneu) {
    const box = document.getElementById('eqBox'); if (!box) return;
    box.innerHTML = '<div style="line-height:1.85;">' +
      '<div><strong>Stiffness switch:</strong> E(T) sigmoid, E_active = ' + Ea.toFixed(1) + ' MPa → E_passive = ' + (E_PASSIVE / 1000).toFixed(1) + ' GPa across T_g = 40 °C</div>' +
...
(function () {
  ...
  function refresh() {
    ...
    const li = document.getElementById('liveInsight');
    if (li) li.innerHTML = '<strong>Live Insight:</strong> Each stroke extends ' + dExt.toFixed(1) + ' mm. With μ_asym = <strong>' + asym.toFixed(2) + '</strong> the robot nets <strong>' + dNet.toFixed(2) + ' mm/cycle</strong> → <strong>' + v.toFixed(2) + ' mm/s ' + (dNet > 0.05 ? 'forward' : dNet < -0.05 ? 'backward' : 'and just oscillates in place') + '</strong>. ' + (Math.abs(asym - 1) < 0.05 ? 'Symmetric friction gives zero net travel — the classic smooth-surface failure.' : '');
  ...
  function updateEq(dExt, dNet, asym, v) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div style="color:#8b929b;font-size:0.92em;">A smooth printed surface has μ_fwd ≈ μ_back ⇒ δ_net ≈ 0. Asymmetry requires printed features / angled legs.</div></div>';
  }
...
(function () {
  ...
  function refresh() {
    ...
    if (li) li.innerHTML = hold
      ...
      : '<strong>⚠ Live Insight:</strong> At θ = ' + th.toFixed(0) + '° the cosθ term cuts each finger to <strong>' + F.toFixed(2) + ' N</strong> — closure needs ' + (need / mu).toFixed(2) + ' N to hold ' + m.toFixed(0) + ' g. ' + (th > 80 ? 'As θ→90° the normal force vanishes and no pressure can save the grip.' : 'The object <strong>drops</strong>.');
  ...
  function updateEq(F, Rmin, Rmax, need, hold) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div><strong>Force closure:</strong> F·μ_contact ' + (hold ? '>' : '≤') + ' m·g/3 = ' + need.toFixed(2) + ' N ⇒ ' + (hold ? '<span style="color:#15803D;font-weight:700;">HOLD</span>' : '<span style="color:#B42318;font-weight:700;">DROP</span>') + '</div>' +
      '<div style="color:#8b929b;font-size:0.92em;">cos(90°)=0: at θ=90° the normal force → 0, so closure fails regardless of pressure.</div></div>';
  }
...
(function () {
  ...
  function updateEq(mt, tuned) {
    ...
    box.innerHTML = '<div style="line-height:1.85;">' +
      ...
      '<div><strong>Response:</strong> overshoot = ' + mt.os.toFixed(0) + ' %, t_s(2%) = ' + (isFinite(mt.ts) ? mt.ts.toFixed(2) + ' s' : 'unstable') + ', &omega;_cl ≈ ' + (isFinite(mt.wcl) ? mt.wcl.toFixed(1) + ' rad/s' : '—') + ' ⇒ ' + (tuned ? '<span style="color:#15803D;font-weight:700;">TUNED</span>' : '<span style="color:#B42318;font-weight:700;">RETUNE</span>') + '</div>' +
