...
Conventional robots are built from rigid links and joints; soft robots instead deform their whole body to move, grip, and adapt. When those bodies are 3D-printed from stimuli-responsive materials, the printed part isn't the finished robot. Heat, pressure, or a control signal reshapes it afterwards. That post-print, time-dependent transformation is the "fourth dimension." This experiment builds a soft finger up in five layers: make it bend, make it hold that bend for free, make a body crawl, make a hand grip, and finally close a control loop so the bend lands exactly where you want it.

---

## 2. Core Concept: Pressure, Modulus, and Friction as Design Knobs
Three physical levers recur throughout soft-robot design. **Pressure** inflates cavities and generates the moments that bend and grip. **Modulus**, how stiff the material is, decides whether a shape springs back or stays put, and in shape-memory polymers it can be switched by temperature. **Friction**, and specifically its directional asymmetry, is what turns symmetric body oscillation into net locomotion. Each sub-calculator isolates one of these levers, then Sub-Calc E ties the pneumatic actuator back into a feedback loop.
...
<div align="center" style="font-size: 1.1em; font-weight: bold;">&kappa; = M / (E &middot; I<sub>eff</sub>) &nbsp;&nbsp;gives&nbsp;&nbsp; &theta;<sub>lin</sub> = &kappa; &middot; L</div>
...
The coefficient &beta; is calibrated so that 20 kPa gives 28&deg; and 80 kPa gives 94&deg;: equal pressure steps buy ever less angle as the finger curls.
...
The shape fixity R<sub>f</sub> is the fraction of the bend that survives unloading (as in the SMP programming cycle of Exp 1). The payoff is energetic: holding pneumatically leaks power continuously at P&middot;Q<sub>leak</sub>, whereas once T &lt; T<sub>g</sub> the SMP-locked hold costs **exactly 0 W**. If the finger never crosses T<sub>g</sub>, releasing pressure lets it spring straight back, with no lock.
...
The cos&theta; term is the whole story: as &theta; approaches 90&deg; the normal force vanishes and no amount of pressure can save the grip, so the object drops. Inverting the closure condition gives the heaviest object three fingers can hold, m<sub>max</sub> = 3&middot;F<sub>grasp</sub>&middot;&mu;/g.
...
A PID controller drives the valve command u from the tracking error e = &theta;<sub>set</sub> &minus; y, with the command clamped to the 0-150 kPa valve range (and integral anti-windup at the rails):
...
The step response is scored for percent overshoot and 2 % settling time, with a second-order estimate of damping &zeta; (from the overshoot) and closed-loop bandwidth &omega;<sub>cl</sub> &asymp; 4/(&zeta;&middot;t<sub>s</sub>). A well-tuned loop meets overshoot &lt; 15 % and t<sub>s</sub> &lt; 2 s; too much K<sub>p</sub> rings, too little never catches the setpoint. Since the plant model is linear, it over-predicts on large steps where the real finger saturates, so these gains are a starting point, not the last word on hardware.
...
## 8. Closed-Loop Motion in Real Time, with Stiffness on Demand

A soft robot is defined by what it does *over time* under feedback. The pneumatic segment is a first-order
plant, &tau;<sub>p</sub>&middot;dy/dt = K<sub>plant</sub>&middot;u &minus; y, and the PID law u =
K<sub>p</sub>(e + (1/T<sub>i</sub>)&int;e dt + T<sub>d</sub> de/dt) drives it toward &theta;<sub>set</sub>
with a measurable overshoot and a 2% settling band. Locomotion is time too: crawling velocity v =
&delta;<sub>net</sub>&middot;f is stroke times cycle frequency, and grasping needs instantaneous force
closure F<sub>grasp</sub>&middot;&mu;<sub>contact</sub> &gt; mg/3. Real-time behaviour is the fourth
dimension of the robot.

**Stiffness locking as a second timescale.** Shape-memory stiffening adds a slow clock on top of the fast
control loop: E(T) rises through T<sub>g</sub> so the limb can hold a pose after it's reached, a
self-locking, variable-stiffness response the controller can call on demand.

<div align="center" style="font-size: 1.1em; font-weight: bold;">tuning passes only if it settles within the band without valve saturation (bad gains ring or clip)</div>

**Bench bridge.** The P/PI/PID controller acting on a first-order thermal plant is exactly the Control
Systems *temperature control of an oven* experiment (IARE). Mapped page in `screenshots_references/`.
