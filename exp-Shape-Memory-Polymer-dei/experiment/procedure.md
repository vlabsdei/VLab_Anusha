...
**Objective:** Observe the transition from a glassy state to a rubbery state and calculate the temperature-dependent molecular relaxation shift factor $\log(a_T)$.

1. **Select Base Material:** In the left sidebar, choose a base polymer preset (e.g., **PU-SMP**, **PLA-SMP**, or **PMMA-SMP**). Each has a unique Glass Transition Temperature ($T_g$).
2. **Set Variables:** Adjust the **Heating Rate (°C/min)** and **Test Temperature (°C)** sliders.
3. **Initiate Cycle:** Click **Run Thermal Cycle**.
4. **Observe Kinetics:**
   * Watch the 3D viewport: polymer chains will transitions from rigid vibration (blue-grey) to wave-like writhing and high mobility (warm burgundy) as the temperature crosses $T_g$.
   * Analyze the plots: the linked cursor tracks WLF $\log(a_T)$ shift factor and DSC heat capacity inflection points in real-time.
5. **Record Results:** Observe the calculated WLF shift factor ($a_T$) and the active thermal regime.
...
1. **Deformation Temp:** Set the **Programming Temp (°C)** slider above the material's $T_g$.
2. **Apply Stress:** Adjust the **Applied Stress (MPa)** slider (range: $0.5\text{--}4.0\text{ MPa}$).
...
5. **Verify Fixity:** Observe the calculated **Strain Fixity Ratio ($R_f$)** and the locked **Programmed Strain ($\epsilon_u$)**.
...
2. **Set Opposing Load:** If Constrained, adjust the **Opposing Stress (MPa)** slider (range: $0.0\text{--}5.0\text{ MPa}$).
3. **Set Actuation Variables:** Adjust the **Recovery Temp (°C)** and **Reference Relaxation Time $\tau_{\text{ref}}$ (s)**.
...
6. **Record Parameters:** Check the Shape Recovery Ratio ($R_r$), blocking recovery stress, and timescale to 95% recovery ($t_{95}$).
...
1. **Set Blend Fraction:** Use the weight fraction slider to adjust the PU polymer weight fraction $w_1$ (which dynamically calculates PMMA weight fraction $w_2$).
...
4. **Observe Recovery Profile:**
   ...
   * In **Miscible Blend Mode**, the specimen displays a single-stage, rapid recovery transition at a unified blend glass transition temperature ($T_{g,\text{blend}}$).
...
2. **Set Thermal Properties:** Adjust the **Specific Heat Capacity ($C_p$)** and **Material Density ($\rho$)** sliders.
3. **Trigger Actuation:** Click **Run Load Cycle**.
4. **Observe Output:** Watch the 3D actuator heat up to trigger temperature ($45^\circ\text{C}$), lift a mechanical load block, and perform work.
5. **Record Energy Budget:** Note the output values in the results panel:
   * **Programmed Stored Energy ($U_{\text{stored}}$)**
