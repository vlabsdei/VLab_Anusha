...
A hydrogel is a crosslinked polymer network that soaks up water yet holds its shape. What makes it a 4D-printing material is that the amount of water it holds is not fixed - it responds to its surroundings. Change the solvent, the pH, or the temperature and the network swells or collapses, and because a printed part is anisotropic that volume change becomes directed motion: a fold, a valve stroke, a burst of drug. The "fourth dimension" here is the programmed shape change a flat-printed gel performs on its own once placed in the body or a beaker. This lab works through five such devices, each governed by a different piece of polymer physics but all rooted in the same swelling thermodynamics.
...
For a neutral gel in a good solvent, the equilibrium polymer volume fraction v<sub>p</sub> is the root of the **Flory&ndash;Rehner equation**, which sets the mixing terms against the elastic term:
...
Below about 30&deg; the gripper stays flat and fails; past 160&deg; it closes fully on the object. Lower &chi; or looser crosslinking both raise Q and tighten the fold.
...
A capsule shell built from a weak polyacid stays neutral - and compact - in stomach acid, then charges up as the pH climbs past its pKa. The fraction of ionised groups follows **Henderson&ndash;Hasselbalch**:
...
The opening fraction is Q normalised between its sealed value at pH 1 and its fully-open value at pH 8, and the capsule is said to spring open at the pH where that fraction reaches 50% (p50). Match p50 to the target organ - PMAA (pKa 4.3) opens earliest, a custom pKa 6.0 shell holds until deeper in the intestine. There is a failure mode: if f<sub>ion</sub> is too high the shell over-swells past &sim;8&times; its sealed volume (Q > 24) and ruptures, dumping the whole dose at once.
...
The Boltzmann constant is k<sub>B</sub> = 1.380649 &times; 10<sup>-23</sup> J/K and T is in kelvin. The design target is native articular cartilage, which spans roughly 0.1&ndash;1 MPa; the tissue map also places brain (&sim;0.001&ndash;0.01 MPa), muscle (&sim;0.01&ndash;0.1 MPa) and skin (&sim;0.1&ndash;2 MPa) for comparison. Because E falls with swelling, a more watery scaffold dents deeper - so the recipe is to tighten crosslinking until the swollen E lands inside the cartilage window.
...
Once a drug-loaded implant is in place, the early-time release fraction follows the empirical **Korsmeyer&ndash;Peppas power law**:
...
Inverting the power law gives the dosing timescales - the half-dose time is t<sub>50</sub> = (0.5/k)<sup>1/n</sup>, and the model is only trusted up to the 60% cutoff, beyond which geometry and boundary effects the power law ignores start to dominate.
