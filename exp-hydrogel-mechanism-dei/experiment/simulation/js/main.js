...
(function () {
    ...
    function gripState(a) { return a < 30 ? { t: 'FAILS to close (stays flat)', c: '#dc2626', fail: true } : a < 110 ? { t: 'Partially curling', c: '#b45309' } : a < 160 ? { t: 'Grips the object', c: '#16a34a' } : { t: 'Fully closed', c: '#16a34a' }; }
    ...
    function refresh() {
        ...
        if (!swelling) { const shown = animP > 0.5; const fail = shown && g.fail; stateLabel.innerText = shown ? 'Gripper: ' + g.t : 'Gripper: flat (just printed, dry)'; stateLabel.style.background = fail ? '#fee2e2' : (shown ? '#dbeafe' : '#f1f5f9'); stateLabel.style.color = fail ? '#b91c1c' : (shown ? '#1e40af' : '#475569'); }
    ...
    xlinkInput.addEventListener('input', () => { if (swelling) stopRun(); r = parseFloat(xlinkInput.value); valXlink.innerText = (r < 0.004 ? 'loose' : r < 0.007 ? 'medium' : r < 0.02 ? 'tight' : 'overtight (locks the gel)') + ' (' + r.toFixed(4) + ')'; animP = 1; refresh(); });
...
(function () {
    ...
    const RUPTURE_Q = Q_NEUTRAL * 8;   // beyond ~8x its sealed volume a real shell can't stretch further - it cracks
    ...
    const rupturedAt = (pH, pKa, fi) => Qof(pH, pKa, fi) > RUPTURE_Q;  // over-swelled past the shell's structural limit
    ...
    let dispOpen = 0, targOpen = 0, dispBurst = 0, targBurst = 0, sweeping = false, tAnim = 0;
    ...
    function update3D() {
        ...
        const burst = dispBurst;                        // 0 intact -> 1 ruptured (shell over-swelled past its limit)
        // FAILURE: past the rupture limit the two halves are flung apart and the shell tears open
        const sep = op * GAP + burst * 1.4;
        const tilt = burst * 0.5;                       // halves cant over as they split
        topHalf.position.y = sep; topHalf.rotation.z = tilt;
        botHalf.position.y = -sep; botHalf.rotation.z = -tilt;
        // healthy: grey->green as it opens.  ruptured: overridden to a torn red shell.
        const col = HG.mix(0x9ca3af, 0x34d399, op).lerp(new THREE.Color(0xdc2626), burst);
        shellMatTop.color.copy(col); shellMatBot.color.copy(col);
        shellMatTop.opacity = shellMatBot.opacity = Math.max(0.35, 0.72 - 0.25 * op - 0.15 * burst);
        // a ruptured shell dumps its entire payload at once; otherwise release tracks the opening fraction
        const released = burst > 0.35 ? NB : Math.floor(op * NB);
        beads.forEach((b, i) => {
            const j = Math.sin(tAnim * 4 + b.ph) * 0.02;
            if (i < released) {                          // escaped beads drift outward and fade (burst = flung out hard)
                b.mesh.position.lerpVectors(b.home, b.out, 0.4 + 0.6 * Math.max(op, burst));
                b.mesh.material.opacity = Math.max(0.2, 0.85 - 0.5 * op); b.mesh.material.transparent = true;
                b.mesh.material.color.set(burst > 0.35 ? 0xf87171 : 0xfda4af);
            } else {
    ...
    function refresh() {
        const a = alphaOf(pH, pKa), Q = Qof(pH, pKa, fion), op = openFracOf(pH, pKa, fion);
        const burst = rupturedAt(pH, pKa, fion);        // solver-driven failure: shell over-swelled past its limit
        if (!sweeping) { targOpen = op; targBurst = burst ? 1 : 0; }
        ...
        HG.put(resQ, Q.toFixed(1) + ' x' + (burst ? ' - RUPTURED' : ''), burst ? '#dc2626' : '#E2570F');
        HG.put(resOpenPh, 'pH ' + tw.p50.toFixed(1), site.c);
        HG.put(resRelease, burst ? 'Shell ruptured - dose dumped' : site.txt, burst ? '#dc2626' : site.c);
        HG.put(resWindow, 'pH ' + tw.p10.toFixed(1) + ' - ' + tw.p90.toFixed(1));
        if (!sweeping) {
            const open = op > 0.5;
            if (burst) {
                stateLabel.innerText = 'Capsule RUPTURED: over-swelled (Q ' + Q.toFixed(1) + ' > ' + RUPTURE_Q.toFixed(0) + ') at ' + locationOf(pH);
                stateLabel.style.background = '#fee2e2'; stateLabel.style.color = '#b91c1c';
            } else {
        ...
        liveInsight.innerHTML = '<strong>Live Insight:</strong> At pH ' + pH.toFixed(1) + ' (' + locationOf(pH) + ') the ' + nameOfPka(pKa) + ' shell is ' + (a * 100).toFixed(0) + '% charged and ' + (op * 100).toFixed(0) + '% open. ' +
            (burst ? 'With this many acid groups the Donnan pressure swells it to <strong>' + Q.toFixed(1) + 'x</strong> - past the shell\'s stretch limit, so it <strong>ruptures and dumps the whole dose</strong>. Use fewer ionisable groups.' :
    ...
    function stepRun(dt) {
        ...
        const burst = rupturedAt(pH, pKa, fion); targBurst = burst ? 1 : 0;
        stateLabel.innerText = burst ? 'RUPTURED at ' + locationOf(pH) + ' (pH ' + pH.toFixed(1) + ')' : 'Journey: ' + locationOf(pH) + ' (pH ' + pH.toFixed(1) + ')';
        stateLabel.style.background = burst ? '#fee2e2' : (pH < 3.5 ? '#fef3c7' : '#dcfce7');
        stateLabel.style.color = burst ? '#b91c1c' : (pH < 3.5 ? '#92400e' : '#166534');
        refresh();
    }
    ...
    function loop(now) {
        ...
        dispBurst += (targBurst - dispBurst) * 0.08;
        update3D();
        if (S) { S.controls.update(); S.renderer.render(S.scene, S.camera); }
    }
    ...
    LabGate.arm(function () {
        ...
        refresh(); dispOpen = targOpen; dispBurst = targBurst;
        requestAnimationFrame(loop);
    });
})();
...
(function () {
    ...
    // lid must clear the seat lip by this much before fluid can escape - a weak gel can't
    const SEAL_CLEAR = 0.45;
    // post height is the solver's actuation stroke: collapsed baseline + lift that scales with epsilon(Qsw)
    const liftOf = (phi) => 0.1 + 1.15 * epsAct(Qsw) * phi;
    function update3D() {
        if (!S) return;
        const phi = dispPhi;                          // 1 swollen/open -> 0 collapsed/shut
        const strokeH = liftOf(phi);                  // post height = actual actuation stroke from Qsw
        const flowGap = strokeH - SEAL_CLEAR;         // how far the lid has cleared the seat (<0 => still sealed)
        ...
        flow.forEach((f) => {
            ...
            if (f.p < 0.5) {                           // rising up the centre of the pipe
                ...
                if (flowGap < 0.02) f.p = 0.5;         // FAILURE/shut: lid never cleared the seat -> fluid stalls
    ...
    // does the gel ever generate enough stroke to unseat the lid (at full swelling, phi=1)?
    const canEverOpen = () => liftOf(1) - SEAL_CLEAR >= 0.02;
    ...
    function refresh() {
        ...
        const fw = fwhm(Qsw, w), weakGel = !canEverOpen(), open = T < LCST && !weakGel;
        HG.put(resQt, Q.toFixed(2) + ' g/g');
        HG.put(resStroke, (epsAct(Qsw) * 100).toFixed(1) + ' %', weakGel ? '#dc2626' : '#E2570F');
        HG.put(resVcr, VCRof(Qsw).toFixed(1) + ' x', '#E2570F');
        HG.put(resFWHM, fw.toFixed(1) + ' C', fw <= 5 ? '#16a34a' : '#b45309');
        HG.put(resState, weakGel ? 'FAILS - never opens (weak stroke)' : (open ? 'OPEN (flowing)' : 'SHUT (blocked)'), weakGel ? '#dc2626' : (open ? '#16a34a' : '#b45309'));
        if (!sweeping) {
            if (weakGel) {
                stateLabel.innerText = 'Valve FAILS: stroke ' + (epsAct(Qsw) * 100).toFixed(0) + '% too weak to lift the lid';
                stateLabel.style.background = '#fee2e2'; stateLabel.style.color = '#b91c1c';
            } else {
        ...
        liveInsight.innerHTML = '<strong>Live Insight:</strong> At ' + T.toFixed(1) + ' C the gel sits at Q = ' + Q.toFixed(1) + ' g/g, so the valve is <strong>' + (weakGel ? 'STUCK SHUT' : (open ? 'OPEN' : 'SHUT')) + '</strong>. ' +
            (weakGel ? 'This gel only swells to Q<sub>sw</sub> = ' + Qsw.toFixed(1) + ', a mere ' + (epsAct(Qsw) * 100).toFixed(0) + '% stroke - the posts can\'t lift the lid off its seat, so <strong>no fluid ever flows even when cold</strong>. Pick a higher-swelling gel.' :
    ...
    function stepRun(dt) {
        ...
        const weakGel = !canEverOpen(), opn = T < LCST && !weakGel;
        stateLabel.innerText = weakGel ? 'Warming: ' + T.toFixed(1) + ' C - valve STUCK SHUT (weak stroke)' : 'Warming: ' + T.toFixed(1) + ' C - valve ' + (opn ? 'OPEN' : 'SHUT');
        stateLabel.style.background = weakGel ? '#fee2e2' : (opn ? '#dbeafe' : '#fde68a');
        stateLabel.style.color = weakGel ? '#b91c1c' : (opn ? '#1e40af' : '#92400e');
        refresh();
    }
